...
public class ClienteController {
	...
	// capturar los datos del objeto cliente (Nueva forma)
	@PostMapping("/cliente")
	public String capturaCliente(@ModelAttribute("cliente") Cliente cliente, Model model,
	 ...
	 {// capturar
		...
		if (cliente.getNombre() != null && !cliente.getNombre().equals("") && cliente.getApellido() != null
			...
			System.out.println(
					"nombre " + cliente.getNombre() + "apellido " + cliente.getApellido() + "email " + cliente.getEmail());
			
			//persistencia
			clienteService.save(cliente);

			// Pasar parametro a otra ruta de controller
			redirectAttributes.addFlashAttribute("msgOK", "Parámetros correctos");
